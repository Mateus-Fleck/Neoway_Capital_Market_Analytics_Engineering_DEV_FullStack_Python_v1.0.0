    ...
    with open(arquivo_input_novo, mode='r', encoding='utf-8', newline='') as csv_file:
        ...
        with ThreadPoolExecutor() as executor:
            for i, row in enumerate(csv_reader, start=1):
                ...
                if i % 10 == 0 or i == total_empresas:  # Salva os resultados a cada 10 iterações ou na última iteração
                    salvar_resultados_parciais(resultados, arquivo_output_completo)
                    resultados = []  # Limpa a lista de resultados

                # Cálculo do tempo estimado restante
                elapsed_time = time.time() - start_time
                estimated_total_time = (elapsed_time / i) * total_empresas
                remaining_time = estimated_total_time - elapsed_time
                print(f'Tempo total de processamento até agora: {elapsed_time:.2f} segundos')
