...
# Função para buscar CNPJ usando proxies
def buscar_cnpj_google(nome_empresa, proxy):
    try:
        query = f'{nome_empresa} CNPJ'
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
        response = requests.get(f"https://www.google.com/search?q={query}", headers=headers, proxies={"http": proxy, "https": proxy})
        ...
        if response.status_code == 200:
            ...
            time.sleep(10)
            return buscar_cnpj_google(nome_empresa, proxy)
        else:
...
# Função para salvar resultados parciais
...
# Função para nomear o arquivo de saída com versão
def nome_arquivo_output(arquivo_output_completo):
    if os.path.exists(arquivo_output_completo):
        root, ext = os.path.splitext(arquivo_output_completo)
        version = 1
        while os.path.exists(f"{root}_v{version}{ext}"):
            version += 1
        return f"{root}_v{version}{ext}"
    else:
        return arquivo_output_completo
...
arquivo_output_completo = os.path.join(diretorio_output, nome_arquivo_output(arquivo_output))
        ...
        with ThreadPoolExecutor() as executor:
            for i, row in enumerate(csv_reader, start=1):
                if row:  # Verifica se a linha não está vazia
                    ...
                    future = executor.submit(buscar_cnpj_google, nome_empresa, proxy)
