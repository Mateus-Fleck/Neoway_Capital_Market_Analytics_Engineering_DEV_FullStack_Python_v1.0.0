...
def persist_to_bigquery(df, table_id, credentials_path):
    ...
    try:
        ...
        # Ajustar o DataFrame para corresponder ao schema
        df = df[schema]
...
def merge_stock_info(wallet_df, stock_info_df):
    ...
    final_columns = [
        'country', 'name', 'full_name', 'symbol', 'ticker_br', 'snome', 'sector', 
        'industry', 'address', 'city', 'state', 'zip', 'website', 'class_exchange', 
        'research_cnpj', 'longBusinessSummary'
    ]
...
def process_data():
    ...
    persist_to_bigquery(selected_columns_br, 'fluent-outpost-424800-h1.1_raw_Neoway_Capital_Market_Analytics.raw_wallet_br', credentials_path)
    ...
    persist_to_bigquery(final_df, 'fluent-outpost-424800-h1.1_raw_Neoway_Capital_Market_Analytics.raw_wallet_br', credentials_path)
