...
output_directory = os.path.join(os.getcwd(), 'src', 'backend', '1_extract')
wallet_br_path = os.path.join(output_directory, 'wallet_br.csv')
historical_stock_price_path = os.path.join(output_directory, 'historical_stock_price.csv')
...
def create_gcs_folder(bucket_name, folder_name):
    """Cria uma pasta no Google Cloud Storage."""
    client = storage.Client()
    bucket = client.get_bucket(bucket_name)
    blob = bucket.blob(folder_name + '/')  # Adiciona uma barra para indicar uma pasta
    blob.upload_from_string('')  # Faz upload de um arquivo vazio para criar a pasta
...
def main():
    ...
    # Cria a pasta no Google Cloud Storage
    create_gcs_folder('your_bucket_name', 'your_folder_name')
