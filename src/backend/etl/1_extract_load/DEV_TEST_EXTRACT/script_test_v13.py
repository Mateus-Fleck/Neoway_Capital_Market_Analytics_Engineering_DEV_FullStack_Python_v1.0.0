...
def get_stock_info_parallelized(tickers):
    ...
    with Pool() as pool:
        with tqdm(total=total_processes) as pbar:
            data = []
...
def get_historical_data_parallelized(tickers):
    ...
    with Pool() as pool:
        with tqdm(total=len(tickers)) as pbar:
            historical_data = []
            for data in pool.imap_unordered(get_historical_data_parallel, tickers):
...
def process_data():
    ...
    persist_to_bigquery(selected_columns_br, 'project.dataset.raw_wallet_br', credentials_path)
    ...
    persist_to_bigquery(address_df, 'fluent-outpost-424800-h1.1_raw_Neoway_Capital_Market_Analytics.raw_address', credentials_path)
